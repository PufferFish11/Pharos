...
def calculate_distance(bbox, class_label, focal_length, real_widths):
    ...
    print(f"Class: {class_label}, Pixel Width: {pixel_width}, Real Width: {real_width}, Distance: {distance:.2f} m")
...
while True:
    ...
    for det in detections:
        ...
        if label in real_widths:
            distance = calculate_distance((x1, y1, x2, y2), label, focal_length, real_widths)
            
            # If the object is within 1 meter, provide a warning
            if distance < 1.0:
                current_frame_objects.add(label)  # Track the object in the current frame
                if label not in warned_objects:
                    warning_message = f"Warning: {label} detected!"
                    print(warning_message)
                    play_audio_warning(warning_message)  # Play audio in a separate thread
            ...
            cv2.putText(frame, f"{label}: {distance:.2f} m", (int(x1), int(y1)-10),
